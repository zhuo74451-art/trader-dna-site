...
fetch('./cinema/portraits-detail-24faa388955e.json').then(r=>r.json()).then(m=>{manifest=m;queue()}).catch(()=>{});
...
function hydrate(person){
 ...
 const src=new URL(m.url,location.href).href;
...
function quizDetail(){
 ...
 const qNum=Number(w.querySelector('.question-index')?.textContent.replace(/\D/g,''))||1;
 const s=typeof state==='object'?state:(()=>{try{return JSON.parse(localStorage.getItem('82trade-trader-dna:standard-v1-2026-09'))}catch{return null}})();
 const total=s?.mode==='full'?54:18,block=Math.floor((qNum-1)/18),local=(qNum-1)%18,phase=Math.floor(local/6),labels=['開始選擇','繼續探索','輪廓將成'];
 const chosen=Array.from({length:18},(_,i)=>s?.answers?.[block*18+i+1]||null),sig=qNum+':'+chosen.join('');if(w.dataset.detailSig===sig)return;
 if(lastQuizNumber!==qNum){lastQuizNumber=qNum;requestAnimationFrame(()=>scrollTo({top:0,behavior:'instant'}))}w.dataset.detailSig=sig;w.dataset.chapter=String(phase+1);w.classList.add('detail-quiz');
 let steps=w.querySelector('.detail-chapters');if(!steps){steps=document.createElement('ol');steps.className='detail-chapters';w.querySelector('.meta').after(steps)}
 steps.style.setProperty('--chapter-fill',`${((local%6)/6)*100}%`);steps.setAttribute('aria-label','本組答題階段');steps.innerHTML=labels.map((label,i)=>`<li class="${i===phase?'is-current':i<phase?'is-complete':''}" ${i===phase?'aria-current="step"':''}><span>${String(i+1).padStart(2,'0')}</span><b>${label}</b><small>${i<phase?'已走過':i===phase?'進行中':'即將進入'}</small></li>`).join('');
 ...
 const answered=chosen.filter(Boolean).length,cells=chosen.map((v,i)=>`<li class="${v?'is-recorded':''} ${i===local?'is-current':''}" data-slot="${i}"><small>${String(i+1).padStart(2,'0')}</small><b>${v||'—'}</b></li>`).join('');
 panel.innerHTML=`<div class="register-top"><span>YOUR DECISIONS</span><b>${String(block+1).padStart(2,'0')} / SCAN</b></div><div class="register-title"><h3>每一次選擇，<br>都留下痕跡。</h3><span class="register-completed"><b>${String(answered).padStart(2,'0')}</b><small>/ 18 已記錄</small></span></div><ol class="decision-cells" aria-label="已記錄 ${answered} 個選擇">${cells}</ol><div class="register-trace" aria-hidden="true">${window.TraderDNAV4?.weaveMarkup?.(chosen,{mode:'register'})||''}</div><div class="register-footer"><span>沒有標準答案</span><span>只有你的選擇</span></div>`;
 ...
 w.querySelector('.question').id='decision-question';w.querySelector('.options').setAttribute('aria-labelledby','decision-question');w.querySelector('.options').setAttribute('role','group');w.querySelector('.question').setAttribute('tabindex','-1');w.querySelector('.count').setAttribute('aria-label',`第 ${qNum} 題，共 ${total} 題`);if(local===6||local===12)w.classList.add('chapter-arrival');if(lastKeyboardChoice){w.querySelector('.question').focus({preventScroll:true});lastKeyboardChoice=false}
}
let lastKeyboardChoice=false,lastQuizNumber=0;
root.addEventListener('click',ev=>{const opt=ev.target.closest('.option');if(!opt||opt.disabled)return;lastKeyboardChoice=ev.detail===0;const w=opt.closest('.quiz-wrap');if(!w)return;w.dataset.responding='1';const slot=w.querySelector('.decision-cells .is-current');if(slot){slot.classList.add('is-recorded','just-recorded');slot.querySelector('b').textContent=opt.dataset.choice}const cap=w.querySelector('.captured');if(cap)requestAnimationFrame(()=>{if(cap.isConnected)cap.textContent='已記錄，下一個選擇。'})},{capture:true});

document.addEventListener("keydown",ev=>{if(ev.altKey||ev.ctrlKey||ev.metaKey||ev.repeat||document.querySelector("dialog[open]")||ev.target.closest?.("input,textarea,select,[contenteditable]"))return;const k=ev.key.toUpperCase();if(k!=="A"&&k!=="B")return;const b=root.querySelector(`.quiz-wrap .option[data-choice="${k}"]:not(:disabled)`);if(b){ev.preventDefault();lastKeyboardChoice=true;b.click()}});

function sync(){queued=false;landing();result();quizDetail();root.querySelectorAll('.person').forEach(enrich);materialSync();document.body.dataset.scene=root.querySelector('.result')?'result':root.querySelector('.quiz-wrap')?'question':root.querySelector('.reveal')?'reveal':'landing'}
...
addEventListener('beforeunload',()=>{if(exportURL)URL.revokeObjectURL(exportURL)});window.CinemaEdition={revision:'detail-03',poster,refresh:queue};
})();
