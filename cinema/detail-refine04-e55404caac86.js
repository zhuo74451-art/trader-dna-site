...
async function portraitManifest(){
  ...
  portraitManifestPromise=fetch('./cinema/portraits-detail04-card-0dca88dce1bb.json',{cache:'force-cache'})
...
async function portraitSource(name){
  ...
  return bundled; /* Cinema uses only locally bundled portraits. */
}
...
async function hydratePortrait(figure,name){
  ...
  img.src=source.cardUrl||source.url;
  if(img.complete)reveal();
  // The archive adapter owns the image credit.
}
