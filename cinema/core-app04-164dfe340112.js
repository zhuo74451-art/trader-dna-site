...
function restoreSaved(saved){
 ...
 state={...state,...saved};
 const answered=Object.keys(state.answers||{}).length,total=state.mode==='quick'?18:54;
 if(state.completed||answered>=total)renderResult();else{state.index=answered;renderQuestion()}
 return true;
}
function renderLanding(){
 setStatus('SCAN 01 · READY');
 const saved=loadState(),answered=Object.keys(saved?.answers||{}).length;
 const resumeLabel=saved?.completed?'查看上次結果':answered?('繼續上次進度 · '+String(answered).padStart(2,'0')+'/18'):'';
 view.innerHTML=`<section class="hero">
   <div class="hero-main">
     ...
     <div class="cta"><button class="btn" id="start">開始 SCAN 01 <span>→</span></button>${resumeLabel?`<button class="btn alt hero-resume" id="resume">${resumeLabel} <span>↗</span></button>`:''}</div>
 ...
 const resume=document.querySelector('#resume');if(resume)resume.onclick=()=>restoreSaved(loadState());
}
...
async function boot(){
 try{
   ...
   const directStart=new URLSearchParams(location.search).get('start')==='scan01';
   if(directStart&&saved)restoreSaved(saved);
   else if(directStart)start('quick');
   else renderLanding();
